...
class VideoTransform:
    def __init__(self, subset):
        if subset == "train":
            ...
            # self.a = FunctionalModule(lambda x: x / 255.0)
            # self.b = torchvision.transforms.RandomCrop(88)
            # self.c = torchvision.transforms.Grayscale()
            # self.d = AdaptiveTimeMask(10, 25)
            # self.e = torchvision.transforms.Normalize(0.421, 0.165)
    ...
    def __call__(self, sample):
        ...
        # rtype: T x 1 x H x W
        # a = time.time()
        # sample = self.a(sample)
        # b = time.time()
        # # print("1", b - a)
        # sample = self.b(sample)
        # c = time.time()
        # # print("2", c - b)
        # sample = self.c(sample)
        # d = time.time()
        # # print("3", d - c)
        # sample = self.d(sample)
        # e = time.time()
        # # print("4", e - d)
        # sample = self.e(sample)
        # f = time.time()
        # # print("5", f - e)
...
class TextTransform:
    ...
    def __init__(
        ...
        if list(self.hashmap.keys())[0] != '<blank>' and list(self.hashmap.keys())[-1] != '<eos>':
            self.token_list = ["<blank>"] + list(self.hashmap.keys()) + ["<eos>"]
        elif list(self.hashmap.keys())[0] != '<blank>':
            self.token_list = ["<blank>"] + list(self.hashmap.keys())
        elif list(self.hashmap.keys())[-1] != '<eos>':
            self.token_list = list(self.hashmap.keys()) + ["<eos>"]
        else:
            self.token_list = list(self.hashmap.keys())
        self.ignore_id = -1
