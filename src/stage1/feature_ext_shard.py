...
# -*- coding: utf-8 -*-
"""
Stage1 Personality Feature Training - DataParallel version (v2)
Fixes & Enhancements:
- MI loss changed to NON-NEGATIVE margin form => safe with uncertainty weighting (avoid log_vars divergence/NaN)
- TarFile LRU cache with close on eviction + close_all at exit => avoid file descriptor leak
- Cached video frame counting (cv2/av) => less repeated open
- Rich prints: bad ratio, skipped batches, NaN counts, flow missing ratio per backend (dir/npy/shard),
  loss component averages, uncertainty weights/log_vars, MI ramp strength
- Remove unnecessary .clone() to reduce CPU overhead

NOTE:
- You said shard internal frames start from flow_000000.*, so shard member naming keeps i:06d (0-based).
"""
...
# -------------------------
def parse_args():
    p = argparse.ArgumentParser("Stage1 Personality Feature Training - DP + ExternalVal + AdaptiveLoss + MI + FlowShards (v2)")
    ...
    p.add_argument("--exp_name", type=str, default="personality_stage1_dp_v2")
    ...
    # tar cache
    p.add_argument("--tar_cache_max_open", type=int, default=8, help="max open tar files per worker process")
...
# =========================
# 3.1) Flow shard helpers - must match to_sherd.py
# =========================
...
# =========================
# 3.2) Tar LRU cache (FIX: close on eviction + close_all at exit)
# =========================
class TarLRUCache:
    ...
    def get(self, tar_path: str) -> tarfile.TarFile:
        t = self._cache.get(tar_path, None)
        if t is not None:
            self._cache.move_to_end(tar_path, last=True)
            return t

        t = tarfile.open(tar_path, "r")
        self._cache[tar_path] = t
        self._cache.move_to_end(tar_path, last=True)

        while len(self._cache) > self.max_open:
            old_path, old_tar = self._cache.popitem(last=False)
            try:
                old_tar.close()
            except Exception:
                pass
    ...
    def close_all(self):
        for _, t in list(self._cache.items()):
            try:
                t.close()
            except Exception:
                pass
...
def _read_tar_member_bytes(tar: tarfile.TarFile, name: str) -> bytes:
    try:
...
def decode_jpg_bytes_to_rgb_float(b: bytes) -> Optional[np.ndarray]:
    if not b:
        return None
...
def read_flow_from_shards(flow_ref: Dict[str, Any], indices: np.ndarray,
    ...
    """
    return (frames, miss_count)
    NOTE: you confirmed flow names in tar are 0-based, so we keep flow_{i:06d}.
    """
    H, W = fallback_hw
    ...
    tar = _TAR_CACHE.get(flow_ref["tar"])
    key = flow_ref["key"]
    ...
    for i in indices:
        ...
        for ext in try_exts:
            ...
            b = _read_tar_member_bytes(tar, member)
            im = decode_jpg_bytes_to_rgb_float(b)
            if im is not None:
                break
...
# =========================
# 6) Dataset (now returns meta for printing)
# =========================
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if os.path.isdir(vpath):
                ...
                # prefer av count if enabled
                n = 0
                if self.cfg.use_av and _HAS_AV:
                    n = count_video_frames_av(vpath)
                if n <= 0:
            ...
            if isinstance(fref, dict) and fref.get("type", "") == "shard":
                ...
                backend_id = 1
                arr = np.load(fref).astype(np.float32)
                if arr.ndim != 4:
                ...
                # optional: detect NaN/Inf in npy (cheap check on sampled clip only)
                if not np.isfinite(flow_clip).all():
                    raise RuntimeError(f"flow npy has NaN/Inf: {fref}")
                ...
                if len(flows) != self.cfg.flow_frames:
                    ...
                    if len(flows) < self.cfg.flow_frames:
                        ...
                        flows = flows + pad
                        # padded frames are "missing"
                        miss += (self.cfg.flow_frames - len(flows) + len(pad))
                    else:
...
def mi_suppression_margin(ps: torch.Tensor, pd: torch.Tensor, tau: float = 0.07) -> torch.Tensor:
    ...
    Safe to combine with uncertainty weighting (avoid unbounded objective).
    """
    B = ps.size(0)
    if B <= 1:
...
class UncertaintyWeighter(nn.Module):
    ...
    def forward(self, losses: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float]]:
        # create a tensor zero on correct device
...
# =========================
# 10) batch selectors (now keep meta for stats)
# =========================
...
# =========================
# 11) Train / Val / External Val stats printing helpers
# =========================
